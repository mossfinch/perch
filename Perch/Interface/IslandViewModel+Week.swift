...
extension IslandViewModel {
    ...
    func refreshWeek(now: Date = Date()) {
        ...
        Task.detached(priority: .userInitiated) {
            ...
            await MainActor.run { [read] in
                // ⚠️ Only the newest read may publish. Two opens in quick
                // succession start two reads and the slower one can land second
                // with an older week — and because `todayKey` was already
                // written forward, the rollover check would never correct it.
                // Stale over fresh, permanently, with nothing on screen to say so.
    ...
    func correctDay(_ date: String, _ value: Int) {
        ...
        // ⚠️ Invalidate any read already in flight. `DayFlow.read` snapshots the
        // corrections BEFORE it spends ~0.8s walking the week, so a read that
        // started before this press lands after it and replaces this answer with
        // a snapshot that predates it — the press looks like it did nothing.
        // (The value survives on disk and returns on the next open, so it reads
        // as a dead control rather than as lost data, which is worse.)
        weekGeneration &+= 1
    }
}
